...
## Current slice → **— all slices complete**  *(S0–S11 + S13 DONE; S12 DEFERRED to phase 2). Owner launch triggers remain: publish media (`upload-media --go`), exact Fun-English video ids, real-device pass — all data/command-only, no code.*
...
| S13 | Content authoring completion + launch checklist | **DONE** (all 30 lessons authored+validated+audited, media staged/manifest-green + upload tool; owner triggers: publish media, video ids, real-device pass) |
...
- [x] Stage the **full** media payload; `scripts/manifest.mjs` passes with **zero missing keys** (`03 §2.5`, `03 §5.1`). *(All 30 lessons staged to `_media_staging/` — 535 referenced keys, manifest GREEN. The public **upload** to R2 is the owner's licensing trigger: `scripts/upload-media.mjs --go` (dependency-free SigV4; `--selftest` returns 200, dry-run = 381 objects / 1673 MB pending; core-09's 15 keys already live). Publishing the third-party audio is the owner's call per `00 §6` / `03 §9`.)*
...
**Done when:** all 30 weekly lessons are complete (incl. the 60 grammar topics + folded EnglishPod/6ME) and manifest-validated, all lesson JSON + `data/index.json` are in the v2 shape, media resolves 1:1 in staging and is one command from R2 at $0, no analytics identifies a person, and the site renders cleanly across all lesson states. ✅ *(Met 2026-07-19: all 30 validate; 0 critical / 30-minor-all-fixed audit; manifest GREEN 535 keys; render + privacy + budgets pass. **Owner triggers remain: run `upload-media --go` to publish the audio, drop in exact Fun-English video ids, and do the real-device pass** — all data/command-only, no code.)*
