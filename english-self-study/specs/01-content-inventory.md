...
| EnglishPod | 84 mp3 + 28 pdf | ~487 MB | In-lesson speaking half (one per weekly lesson) | [inventory/englishpod.md](inventory/englishpod.md) |
| 6 Minute English | ~30–143 mp3+pdf pairs | ~250 MB–1.5 GB | In-lesson listening/IELTS half (one per weekly lesson) | [inventory/6-minute-english.md](inventory/6-minute-english.md) |
...
The **listening/IELTS-oriented half, folded into each weekly lesson** (as the 6 Minute English section, one episode per lesson, all 30): BBC Learning English episodes under `content/6_minute_english/`, each a ~6-min mp3 + a 4–5 page PDF transcript with a highly consistent, cleanly extractable layout (two-presenter scripted dialogue, a ready-made 3-option quiz with the answer revealed near the end, 6 target vocabulary items, and `INSERT` vox-pop clips of authentic accents — the closest free analogue to real IELTS Section 4 audio). Level is B1–B2; the redefined B1→B2 audience handles them, scaffolded by that week's AJ theme, as the lesson's listening stretch. The `YYMMDD` filename prefix is a 100%-reliable primary key; the middle segment is inconsistent (series-token variants, typos, dropped `english`), so derive the topic slug from the PDF's page-1 title and pair PDF+MP3 by date prefix, not exact stem. The inventory tags ~20 "Yes" core picks (concrete, IELTS Part 1/2 topics) plus reserves and flags abstract episodes to avoid.
...
The **optional grammar reference**: Raymond Murphy's *Essential Grammar in Use* (2nd ed., "with answers", Cambridge University Press), a single ~15.5 MB, ~300-page PDF at `content/grammar_book/`. 114 elementary units organized by grammatical category — but the **redefined curriculum retires the Murphy ladder entirely** (anyone who can follow AJ Hoge is past its A1-late→A2 tier). The book is therefore **not the syllabus**: it is offered only as an **optional reference download**, while on-site grammar is **60 original B1→B2 topics** (two per weekly lesson, 02 §4). It is **copyrighted**: pages are **not** reproduced on the site — we author our own original Uzbek-explained grammar prose and exercises, and reuse just the appendices (irregular verbs, spelling rules) as interactive reference tables. The inventory also documents the **10 L1-interference error clusters** for Uzbek speakers (articles, dropped copula, 3rd-person -s, do-support, present perfect, prepositions, he/she/it gender, SOV word order, have got, plurals/countability) that the Uzbek Grammar Spark sections target explicitly.

| Metric | Value |
|---|---|
| File | 1 pdf, ~15.5 MB, ~300 pages |
| Units | 114 (elementary tier — **retired as the syllabus**; optional reference only) |
