...
> **S13 (as shipped) — the full-course pipeline: faithful scaffold + authored overlay.** Curating 30 lessons at scale split step 2 into a **deterministic assembler** + a small **authored overlay**, so the English immersion input a learner acquires stays byte-faithful to the audio (never paraphrased by an author) while only the Uzbek pedagogy is hand-written. Three scripts were added; the full deterministic run order is now:
> `extract → probe-media → assemble-lesson → compile-grammar → build-index → validate → stage-media → manifest → upload-media`.
> - **`scripts/probe-media.mjs`** (new stage 2b) — `ffprobe` + `stat` every media file a lesson points at (AJ set + the §5 EP/6ME weave + the Murphy PDF) → `data/raw/<id>/media.probe.json` keyed by clean media keys. Every `durationSec`/`bytes` in the lesson JSON is **measured, never guessed**. Git-ignored.
> - **`scripts/assemble-lesson.mjs`** (new stage 2c) — assembles `data/lessons/<id>.json` by merging (a) the **faithful, source-derived scaffold** — `transcripts.{main,vocab,pov}` reflowed from the `.para.json` with the AJ anti-piracy notice + per-page site/page header stripped at the sentence level; `transcripts.sixmin` **reconstructed into clean speaker turns** from `sixmin.txt` (BBC boilerplate dropped, `INSERT` preserved) — a faithful mechanical alternative to the hand-tagged core-09 exemplar; mini-story `{q,a}` pairs (deduped); the EnglishPod dialogue (de-hyphenated) from the draft; and every audio/download node from `media.probe.json` — with (b) the **authored overlay `authoring/lessons/<id>.overlay.json`** (committed source): the two Grammar-Spark topics' metadata + drills, vocab Uzbek glosses, EP Key-Vocab glosses + warm-up + `titleUz`, the 6ME `answerIndex` + `explanationUz` + 6-word pack, `funEnglish`, `speakingPrompt`, intro/can-do/tags. `grammar[].bodyHtml` stays empty here and is filled by `compile-grammar` from `authoring/grammar/<id>-a.md` / `-b.md`. Deterministic, skip-write-if-identical.
> - **`scripts/upload-media.mjs`** (new stage 6) — publishes `_media_staging/` → the R2 bucket behind `media.principiaforge.com` at $0 egress with the immutable `Cache-Control` (§2.4), via a **dependency-free AWS SigV4** implementation (`node:crypto` + global `fetch` — no aws-sdk, no rclone; reads `.env`, never committed). `--selftest` proves auth with a signed HEAD (publishes nothing); the default is a dry-run; `--go` uploads (skipping objects already live with matching size). **Publishing the third-party audio publicly remains the owner's licensing trigger** (00 §6, 03 §9): this ships the mechanism, the owner runs `--go`.
> The **overlay-authoring pattern** (`authoring/lessons/<id>.overlay.json`) joins `authoring/grammar/<id>-{a,b}.md` as the committed authored source; `data/lessons/<id>.json` is a generated artifact. **`funEnglish` ships `id:null`** with a themed channel suggestion (the honest "search on YouTube" state, 04 §5.9); exact video ids are owner-supplied data — a JSON edit, no code change (02 §4, S8).
...
> **`lesson.js` raw-size note.** The per-file budgets above are **gzip** — the authoritative measure for this section. `assets/lesson.js` is **~44.2 KB *raw*** (grew in S11 with the per-track media-error panel + malformed-lesson guards), over the ≤35 KB raw per-file guideline (§4), but only **~13.4 KB gzip** and **lazy-loaded solely on `#/lesson/:id`**, so it never touches first-view JS and stays well within the authoritative gzip budget. If it grows further, split the Lesson-Check / earn logic into a lazily-imported module the way `lesson-episodes.js` / `lesson-speak.js` were extracted (§4) — reviewed in S11 and **not** split (not trivially safe; the gzip metric holds).

> **`method.js` raw-size note.** `assets/method.js` (the bilingual 13-block "How to Study" page) is **~46 KB *raw*** / **~16.2 KB gzip**, over the ≤35 KB raw per-file guideline (§4) but well within the authoritative gzip budget, and **lazy-loaded solely on `#/method`** so it never touches first-view JS. The cleaner long-term fix is to move its bilingual prose out of a JS module and into a data file (content-as-data — the project's core architecture philosophy). **S11 reviewed this and intentionally did not refactor it** (a mechanical split would risk regressions in a hardening pass and buys nothing on the gzip metric); the content-extraction is **deferred** to a dedicated task.

**Tactics:** system fonts (kill the sibling apps' render-blocking third-party font request); YouTube facade; `<audio preload="none">`; lazy per-lesson fetch + `loading="lazy"` images; inline SVG / emoji (no icon font); `immutable` long-cache on media (R2) + manual `?v=N` cache-bust on `app.js`; **boot-chain resource hints** — `<link rel=modulepreload>` for the eager module graph (`core`/`player`/`progress`/`config`, loaded on every route) plus a **language-aware** `<link rel=preload as=fetch crossorigin>` for the i18n dict and, **on the landing route only**, `home.js` + `index.json`, so the boot modules and JSON download in parallel with `app.js` instead of being discovered one RTT apart down a 6-deep serial chain (see the S11 note); host-applied gzip/brotli (JSON compresses ~5–8×). Because content is JSON and the renderer is fixed, JS stays flat as the 30 weekly lessons are authored (and would stay flat past 30).

> **S11 measured budgets (gzip, authoritative — measured, not estimated).** `index.html` **4.00 KB** (≤12) · `styles.css` **14.95 KB** (≤15, after merging the three S11 cluster stylesheets and trimming comments to hold the budget) · `app.js` **6.49 KB** + statically-imported `core.js` **3.10** + `player.js` **4.95** (each ≤35) · `index.json` **0.42 KB** (≤40; 1 lesson) · `core-09.json` **14.7 KB** (≤25). **First meaningful view (Home)** ≈ `index.html` + `styles.css` + `app.js` + `core.js` + `player.js` + `home.js` (3.6) + `progress.js` (9.5) + `index.json` = **~46.5 KB gzip transferred**, comfortably under ~100 KB, with **0 KB fonts** and **0 KB YouTube** pre-tap. On Slow-3G the byte budget is only ~1 s of network, but interactive time is **latency-bound, not byte-/CPU-bound**: the boot chain (`app.js` → `core`/`player` → `config`/`progress` → dict → `home.js` → `index.json`) is otherwise **6 requests deep and serial**, paying one RTT per hop (~4.0 s TTI at 400 ms RTT — the round-2 waterfall CRIT). **S11's round-2 fix flattens it with resource hints** (Tactics above) so every boot resource downloads in parallel. Measured on the CDP rig (gzip origin, cache disabled, `4× CPU`, `400 kbps` + **`400 ms` RTT** Slow-3G, 3 runs): TTI **2.2–2.3 s** — i.e. **interactive < 3 s** with headroom (still <3 s at `500 ms` RTT; ~unchanged at `cpu=1×`, confirming CPU was never the bottleneck), with **no double-fetch** and **no unused-preload warnings**. The `app.js` cache-bust is at **`?v=3`** (S11). Lazy route modules (never first-view): `lesson.js` 13.4 · `method.js` 16.2 · `progress-page.js` 9.7 · `lesson-speak.js` 7.5 · `lesson-episodes.js` 5.1 · others ≤4.8.
