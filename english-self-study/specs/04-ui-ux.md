...
### 4.7 IELTS & CEFR — `#/ielts`  ·  4.8 Grammar Reference — `#/grammar`
...
**Phase accents** (map + badges): Foundation = teal, Momentum = indigo, Fluency = amber — a warm climb from calm→energised. Each is paired with an icon/label so meaning never rests on hue alone. *(Post-launch, the phases are displayed by their English names Foundation / Momentum / Fluency in both languages, and the pace setting is relabelled "Ritm"/"Rhythm" — see 02 §1.)*
...
**Structure & keyboard**
- [x] Semantic landmarks: `<header>`, `<nav>` (top links + bottom nav + sections sheet), `<main>`. *(S11: no **persistent chrome `<footer>`** — the bottom-nav + docked player own that zone on mobile; a global footer would be empty clutter (P1/P5). `<footer>` is reserved for page-level footer content; there is currently no such content (the About tagline that once filled this role was removed with the About page) and it can adopt `<footer>` if any grows back. Landmarks are used correctly for the content that exists.)* One `<h1>` per screen + ordered heading levels verified on all routes (incl. both Home faces + both error screens).
