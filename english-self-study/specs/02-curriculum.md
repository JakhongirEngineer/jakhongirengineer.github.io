...
> **Displayed names (post-launch).** The UI now labels the phases with their **English names — Foundation / Momentum / Fluency — in both UZ and EN modes** (owner's call: they're the target language, clearer, and quietly teach three useful words). The Uzbek renderings *Poydevor / Surʼat / Ravonlik* were dropped from the interface — "surʼat" in particular is one glottal stop from "surat" (picture) and, as a phase label, "speed" never conveyed "momentum". The study-**pace** setting (the Effortless / Sprint / Gentle tracks) is likewise relabelled **"Ritm" (uz) / "Rhythm" (en)** — matching the "7-day rhythm" wording already in the method page. *(The Uzbek names live on only in this design doc, as history.)*
...
- **Fun English** theme + suggested free channel (owner picks the exact video; **never hardcode a video ID** — they die). The pick is **data** (`funEnglish[]` in the lesson JSON), rendered by the S8 facade (04 §5.9); a dead video is a JSON fix, not a code change. *(The `core-09` exemplar carries a **verified embeddable pick** — `QlohNbRUltY` "The Kaizen Mindset", confirmed via YouTube oEmbed — as an owner-swappable placeholder; curation of all 30 is owner-supplied data in S13.)*
